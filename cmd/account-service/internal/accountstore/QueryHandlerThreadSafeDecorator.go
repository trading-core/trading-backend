...
func (decorator *QueryHandlerThreadSafeDecorator) Get(ctx context.Context, input GetInput) (*Account, error) {
	decorator.mutex.Lock()
	defer decorator.mutex.Unlock()
	return decorator.decorated.Get(ctx, input)
}

func (decorator *QueryHandlerThreadSafeDecorator) List(ctx context.Context) ([]*Account, error) {
	decorator.mutex.Lock()
	defer decorator.mutex.Unlock()
