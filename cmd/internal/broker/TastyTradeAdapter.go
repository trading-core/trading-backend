...
func NewTastyTradeAdapter(input NewTastyTradeAdapterInput) *TastyTradeAdapter {
	apiURL, err := url.Parse(input.APIEndpoint)
	fatal.OnError(err)
	return &TastyTradeAdapter{
		apiURL:         *apiURL,
		accountObject:  input.AccountObject,
...
func (adapter *TastyTradeAdapter) GetBalanceInfo(ctx context.Context) (output *BalanceInfo, err error) {
	target := url.URL{
		...
		Path:   fmt.Sprintf("/accounts/%s/balances", adapter.accountObject.BrokerAccountID),
	}
	request, err := http.NewRequestWithContext(ctx, http.MethodGet, target.String(), nil)
	if err != nil {
		panic(err)
	}
	accessToken, err := adapter.getAccessToken(ctx)
	if err != nil {
		return
	}
	...
	var tastyTradeAccountBalance TastyTradeAccountBalance
	err = json.NewDecoder(response.Body).Decode(&tastyTradeAccountBalance)
	if err != nil {
		return
	}
	...
	output = &BalanceInfo{
		AccountBroker: adapter.accountObject.BrokerType,
		Balance:       balance,
...
type TastyTradeAccountBalance struct {
	Data    TastyTradeAccountBalanceData `json:"data"`
	Context string                       `json:"context"`
}

type TastyTradeAccountBalanceData struct {
