...
func TestBreakoutStrategy(t *testing.T) {
	Convey("Given a breakout strategy", t, func() {
		Convey("When lookback is one bar and price breaks session high", func() {
			strategy := tradingstrategy.NewBreakoutStrategy(1)
			decision := strategy.Evaluate(tradingstrategy.EvaluateInput{Price: 101, SessionHighPrice: 100})
		...
		Convey("When lookback is greater than one and lookback high is available", func() {
			strategy := tradingstrategy.NewBreakoutStrategy(5)
			decision := strategy.Evaluate(tradingstrategy.EvaluateInput{Price: 106, SessionHighPrice: 99, LookbackHighPrice: 105})
		...
		Convey("When breakout condition is not met", func() {
			strategy := tradingstrategy.NewBreakoutStrategy(3)
			decision := strategy.Evaluate(tradingstrategy.EvaluateInput{Price: 100, SessionHighPrice: 101, LookbackHighPrice: 0})
